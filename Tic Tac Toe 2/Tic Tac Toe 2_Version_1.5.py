# Tic-Tac-Toe 2!
# Game Version: 1.5
# The code Update! What's new ?:
# code update, and escape logic enhanced for exiting matches
...
def reset_game():
    global board, game_active, current_player, menu_active
    if not menu_active:
        if menu in [3, 4] and not game_active:
            board = [' '] * 9
            game_active = True
            current_player = 1
            menu_active = False
...
def handle_menu():
    global menu, game_active, menu_active, difficulty, mode
    if menu == 1:
...
def handle_moves():
    global board, mode, current_player
        ...
        if not game_active:
            if not menu_active:
                if menu == 4:
                    menu_active = True
                    reset_board()
                    menu = 3 # Set menu to 3 for choosing the mode
                else:
                    menu_active = True
                    reset_board()
...
def handle_input(event):
    global current_player, game_active, menu_active, menu, mode, difficulty
