...
def handle_input():
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
            if not menu_active:
                menu = 4
            if menu == 4 and not game_active:
                reset_game()

        if menu != 1 and event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
            if not game_active:
                menu = menu - 1
...
no_diff_surf = game_font.render('No Difficulty', False, 'Black')
easy_surf = game_font.render('Easy', False, 'Black')
normal_surf = game_font.render('Normal', False, 'Black')
hard_surf = game_font.render('Hard', False, 'Black')
